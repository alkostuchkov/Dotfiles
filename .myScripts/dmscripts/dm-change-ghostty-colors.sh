...
is_color_exists=0
config_file=${CONFIG_STORE_DIR-~/.config/ghostty/config}
prefix=${COLORS_STORE_DIR-~/.config/ghostty/themes}
color_files=("$prefix"/*)
color_files=("${color_files[@]#"$prefix"/}")

if [[ -n $WAYLAND_DISPLAY ]]; then
    # DMENU="wmenu"
    color=$(printf '%s\n' "${color_files[@]}" | wmenu -i -l 10 -f 'IosevkaTerm_IlovePlus 17' -p 'Ghostty colors you want:')
	xdotool="ydotool type --file -"
elif [[ -n $DISPLAY ]]; then
    DMENU="dmenu -i -l 10 -nf ${nf} -nb ${nb} -sf ${sf} -sb ${sb} -fn ${fn} -p"
    color=$(printf '%s\n' "${color_files[@]}" | ${DMENU} 'Ghostty colors you want:')
...
if [[ ${is_color} == "true" ]]
then
    old_line=$(grep "theme =" ${config_file} | awk -F/ '{print $NF}' | cut -d'"' -f1)
    new_line="theme = ${color}"
