...
is_color_exists=0
config_file=${CONFIG_STORE_DIR-~/.config/wezterm/wezterm.lua}
prefix=${COLORS_STORE_DIR-~/.config/wezterm/colors}
color_files=("$prefix"/*.lua)
color_files=("${color_files[@]#"$prefix"/}")
color_files=("${color_files[@]%.lua}")

if [[ -n $WAYLAND_DISPLAY ]]; then
    # DMENU="wmenu"
    color=$(printf '%s\n' "${color_files[@]}" | wmenu -i -l 10 -f 'IosevkaTerm_IlovePlus 17' -p 'WezTerm colors you want:')
	xdotool="ydotool type --file -"
elif [[ -n $DISPLAY ]]; then
    DMENU="dmenu -i -l 10 -nf ${nf} -nb ${nb} -sf ${sf} -sb ${sb} -fn ${fn} -p"
    color=$(printf '%s\n' "${color_files[@]}" | ${DMENU} 'WezTerm colors you want:')
...
# import = ["/home/alexander/.config/alacritty/colors/Ayu-Mirage-Dark.toml"]
    ...
    old_line=$(grep "local my_colors" ${config_file} | awk -F/ '{print $NF}' | cut -d'"' -f1)
    new_line="${color}"
