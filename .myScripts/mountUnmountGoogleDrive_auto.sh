...
gdrive_name="GoogleDrive_auto"
gdrive_path="$HOME/GoogleDrive_auto/"
gdrive_statusfile_path="$HOME/.myScripts/GoogleDriveStatus_auto.txt"
...
if [[ "$is_mounted" ]]; then  # if mounted
    ...
    google-drive-ocamlfuse -label auto "$gdrive_path"  # try to mount GoogleDrive
    if [[ "$?" == 0 ]]; then
