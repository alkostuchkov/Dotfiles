...
local function worker(user_args)
    ...
    weather_api_widget = wibox.widget {
        {
            {
                {
                    {
    ...
    local current_weather_api_widget = wibox.widget {
        {
            {
                {
                    id = 'icon',
    ...
    local function update_widget(widget, stdout, stderr)
        ...
        current_weather_api_widget:update(result.current)

        local final_widget = {
            current_weather_api_widget,
            spacing = 16,
            layout = wibox.layout.fixed.vertical
        }
    ...
    weather_api_widget:buttons(gears.table.join(awful.button({}, 1, function()
        if weather_popup.visible then
            weather_api_widget:set_bg('#00000000')
            weather_popup.visible = not weather_popup.visible
        else
            weather_api_widget:set_bg(beautiful.bg_focus)
            weather_popup:move_next_to(mouse.current_widget_geometry)
        end
    end)))

    weather_api_widget:connect_signal("mouse::enter", function()
        weather_api_widget:set_bg(beautiful.bg_focus)
        weather_popup:move_next_to(mouse.current_widget_geometry)
    end)

    weather_api_widget:connect_signal("mouse::leave", function()
        if weather_popup.visible then
            weather_api_widget:set_bg('#00000000')
            weather_popup.visible = not weather_popup.visible
        end
    ...
    watch(
        ...
        update_widget, weather_api_widget
    )

    return weather_api_widget
end

return setmetatable(weather_api_widget, {__call = function(_, ...) return worker(...) end})
