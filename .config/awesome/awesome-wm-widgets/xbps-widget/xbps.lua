...
local DIR = HOME .. "/.config/awesome/awesome-wm-widgets/xbps-widget/"
...
local function worker(user_args)
    ...
    function xbps_widget.set(new_value)
        xbps_widget:get_children_by_id("txt_updates")[1]:set_text(new_value)
        -- xbps_widget:get_children_by_id("txt_icon")[1]:set_text(" ")
    end
    ...
    -- timer = select(2, awful.widget.watch([[bash -c "xbps-install -nuMS 2>/dev/null"]],
    ...
    -- Set fg and bg colors for xbps_widget
    local xbps_widget_clr = wibox.widget.background()
    xbps_widget_clr:set_widget(xbps_widget)
    xbps_widget_clr:set_fg(fg_color)
    -- xbps_widget_clr:set_bg("#ff0000")
    -- return wibox.container.background(xbps_widget, "#ff0000")

    return xbps_widget_clr
    -- return xbps_widget
end

return setmetatable(xbps_widget, { __call = function(_, ...) return worker(...) end })
