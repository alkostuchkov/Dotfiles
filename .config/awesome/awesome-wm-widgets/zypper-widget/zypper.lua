...
local DIR = HOME .. "/.config/awesome/awesome-wm-widgets/zypper-widget/"
...
local function worker(user_args)
    ...
    function zypper_widget.set(new_value)
        zypper_widget:get_children_by_id("txt_updates")[1]:set_text(new_value)
        -- zypper_widget:get_children_by_id("txt_icon")[1]:set_text(" ")
    end
    ...
    -- timer = select(2, awful.widget.watch([[bash -c "zypper-install -nuMS 2>/dev/null"]],
    timer = select(2, awful.widget.watch([[bash -c "zypper --no-refresh lu --best-effort | grep 'v  |' | awk '{print $3}' 2>/dev/null"]],
    -- timer = select(2, awful.widget.watch([[bash -c "xbps-install -nuMS | awk '{print $1}' 2>/dev/null"]],
        interval,
    ...
    -- Set fg and bg colors for zypper_widget
    local zypper_widget_clr = wibox.widget.background()
    zypper_widget_clr:set_widget(zypper_widget)
    zypper_widget_clr:set_fg(fg_color)
    -- zypper_widget_clr:set_bg("#ff0000")
    -- return wibox.container.background(zypper_widget, "#ff0000")

    return zypper_widget_clr
    -- return zypper_widget
end

return setmetatable(zypper_widget, { __call = function(_, ...) return worker(...) end })
